...
export default function GroverPOC() {
  ...
  const handleRunToy = async () => {
    if (toyBitstring.length !== numQubits || !/^[01]+$/.test(toyBitstring)) {
      toast.error(
        `Target bitstring must be exactly ${numQubits} bits long and contain only 0s and 1s.`,
      );
      return;
    }
    if (backendType === 'ibm_cloud' && !credsSaved) {
      toast.error(
        'Please save your IBM Cloud credentials before running on the QPU.',
      );
      return;
    }
    ...
    try {
      ...
      const msg =
        err.response?.data?.detail || 'Error running quantum POC circuit.';
      toast.error(msg);
    }
    ...
    <div className="max-w-4xl mx-auto">
      <div className="bg-slate-900/60 rounded-3xl p-8 border border-slate-800 shadow-2xl backdrop-blur-md">
        ...
        <div className="flex items-center gap-4 mb-6">
          ...
          <div>
            <h2 className="text-2xl font-bold text-white">
              Quantum Sim Proof of Concept
            </h2>
            <p className="text-slate-400 text-sm mt-1">
              Blind Grover circuit on local simulator or IBM Cloud QPU.
            </p>
          </div>
        </div>
        ...
        <div className="bg-blue-950/40 border border-blue-500/20 rounded-2xl p-4 mb-6 text-sm text-blue-200/80 leading-relaxed">
          ...
          Your query is XOR-encrypted with a random OTP key <em>before</em>{' '}
          leaving your browser. The server runs Grover's on the encrypted bits —
          it cannot determine your original query. You decrypt the result
          locally. <span className="text-amber-400/80">⚠ Caveat:</span> A
          cryptographically rigorous PIR requires an oblivious oracle; this is a
              ...
              <div className="mt-6">
                <label className="block text-sm font-medium text-slate-300 mb-2">
                  ...
                  <span className="text-slate-500 font-normal ml-2">
                    (searching 2<sup>{numQubits}</sup>={2 ** numQubits} states)
                  </span>
                </label>
                    ...
                    <tbody>
                      {pocChunks.map((chunk) => {
                        ...
                        return (
                          <tr
                            key={chunk.index}
                            className={`border-b border-slate-800/30 hover:bg-slate-800/50 transition-colors ${isMatch ? 'bg-amber-900/20' : ''}`}>
                            <td className="px-5 py-2.5 font-mono text-slate-500">
                              {chunk.index}
                            </td>
                            <td
                              className={`px-5 py-2.5 font-mono tracking-widest ${isMatch ? 'text-amber-400 font-bold' : ''}`}>
                              {chunk.bits}
                            </td>
                            <td className="px-5 py-2.5 font-mono text-emerald-400">
                              {bitsToDna(chunk.bits)}
                            </td>
                          </tr>
                        );
                      })}
            ...
            <div className="space-y-5">
              ...
              <div>
                ...
                <input
                  ...
                  onChange={(e) => {
                    if (/^[01]*$/.test(e.target.value))
                      setToyBitstring(e.target.value);
                  }}
                  maxLength={numQubits}
                ...
                <div className="mt-2 text-xs text-slate-400 flex items-center justify-between">
                  <span>
                    Mapped DNA:{' '}
                    <span className="font-mono text-emerald-400 bg-emerald-900/20 px-1 py-0.5 rounded">
                      {bitsToDna(toyBitstring)}
                    </span>
                  </span>
              ...
              <div className="pt-4 border-t border-slate-800">
                ...
                <select
                  ...
                  <option value="simulator">
                    Local Qiskit Simulator (Aer)
                  </option>
                  <option value="ibm_cloud">
                    IBM Cloud QPU (real hardware)
                  </option>
                </select>
              </div>
                  ...
                  <motion.div
                    ...
                    {noiseLevel > 0 && (
                      <p className="text-xs text-amber-500/70 mt-2">
                        ⚠ At {(noiseLevel * 100).toFixed(1)}% error rate, the
                        target state may no longer be the most probable
                        measurement outcome — simulating real NISQ hardware
                        degradation.
                      </p>
                    )}
                  </motion.div>
                ...
                {backendType === 'ibm_cloud' && (
                  <motion.div
                    ...
                    <div className="space-y-3">
                      <div>
                        <label className="block text-xs font-medium text-blue-200/60 mb-1">
                          IAM API Key
                        </label>
                        <input
                          type="password"
              ...
              <button
                ...
                disabled={
                  isToyRunning || (backendType === 'ibm_cloud' && !credsSaved)
                }
            ...
            {pocEncryptionState && (
              <motion.div
                ...
                <div className="bg-slate-900 border border-slate-800 p-6 rounded-2xl shadow-inner">
                  ...
                  <div className="flex flex-col sm:flex-row items-center gap-3 text-center font-mono text-sm">
                    <div className="flex-1 w-full bg-slate-950 p-4 rounded-xl border border-slate-800 shadow-sm">
                      <div className="text-slate-500 text-xs mb-2">
                        Query Bits
                      </div>
                      <div className="text-white text-lg tracking-widest">
                        {pocEncryptionState.bits}
                      </div>
                    </div>
                    <div className="text-slate-500 text-xl font-sans shrink-0 select-none px-1">
                      ⨁
                    </div>
                    <div className="flex-1 w-full bg-slate-950 p-4 rounded-xl border border-slate-800 text-amber-500 flex flex-col justify-center shadow-sm">
                      <div className="text-amber-600/70 text-xs mb-2 font-sans font-medium uppercase tracking-wider">
                        OTP Key
                      </div>
                      <div className="text-lg tracking-widest">
                        {pocEncryptionState.key}
                      </div>
                    </div>
                    <div className="text-slate-500 text-xl font-sans shrink-0 select-none px-1">
                      =
                    </div>
                    <div className="flex-1 w-full bg-blue-900/20 p-4 rounded-xl border border-blue-500/30 text-blue-400 flex flex-col justify-center shadow-sm">
                      <div className="text-blue-500/70 text-xs mb-2 font-sans font-medium uppercase tracking-wider">
                        Encrypted → Server
                      </div>
                      <div className="text-lg tracking-widest font-bold">
                        {pocEncryptionState.encrypted}
                      </div>
                    </div>
                  </div>
                </div>
                  ...
                  <motion.div
                    ...
                    <div className="bg-slate-900 border border-slate-800 p-6 rounded-2xl shadow-inner">
                      <h3 className="text-sm font-bold text-amber-400 mb-5 flex items-center gap-2">
                        <div className="w-6 h-6 rounded bg-amber-500/20 flex items-center justify-center text-xs">
                          2
                        </div>
                      ...
                      toyResult.step_circuits.length > 0 ? (
                        <GroverStepNavigator
                          stepCircuits={toyResult.step_circuits}
                          activeStep={activeStep}
                          onStepChange={setActiveStep}
                          targetBits={
                            pocEncryptionState?.encrypted || toyBitstring
                          }
                        />
                      ) : (
                ...
                {toyResult && (
                  <motion.div
                    ...
                    <div className="flex flex-col sm:flex-row items-center gap-3 text-center font-mono text-sm">
                      ...
                      <div className="flex-1 w-full bg-emerald-900/20 p-4 rounded-xl border border-emerald-500/30 text-emerald-400 flex flex-col justify-center shadow-sm relative overflow-hidden">
                        <div className="text-emerald-500/70 text-xs mb-2 font-sans font-medium uppercase tracking-wider">
                          Decrypted
                        </div>
                        <div className="text-lg tracking-widest font-bold">
                          {xorBits(
